...
OpenClaw Browser maps every interactive element on a page to a stable `@ref` ID. Form filling is a matter of reading the snapshot, identifying the right `@ref` numbers, and calling `browse_fill`, `browse_select`, or `browse_upload_file` for each field. No CSS selectors, no XPath, no fragile DOM traversal.
