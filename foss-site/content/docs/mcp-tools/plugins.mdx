...
description: Extend OpenClaw with WASM plugins and Rhai userscripts
---

OpenClaw Browser supports two extension mechanisms: WASM plugins for compiled extensions (requires the `wasm` feature flag and Wasmtime), and Rhai userscripts for lightweight automation written in a simple scripting language. Both can be loaded at runtime and executed against the current page.
