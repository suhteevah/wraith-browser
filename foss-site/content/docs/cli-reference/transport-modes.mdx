...
description: How OpenClaw Browser communicates with MCP clients — stdio and HTTP/SSE
---

OpenClaw Browser's `serve` command supports two transport modes for MCP
...
1. The MCP client launches `openclaw-browser serve` as a subprocess.
2. The client sends JSON-RPC requests over the process's stdin.
3. OpenClaw writes JSON-RPC responses to stdout.
...
```bash
openclaw-browser serve --transport sse --host 0.0.0.0 --port 3100
```
...
openclaw-browser serve --transport sse

# All interfaces, custom port
openclaw-browser serve --transport sse --host 0.0.0.0 --port 8080

# With verbose logging
openclaw-browser serve --transport sse --host 0.0.0.0 --port 3100 -v
```
