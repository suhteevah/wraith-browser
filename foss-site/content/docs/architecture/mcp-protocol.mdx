...
description: How OpenClaw Browser implements the Model Context Protocol — JSON-RPC over stdio, tool registration, the dispatch model, and transport modes.
---

# MCP Protocol Integration

OpenClaw Browser exposes its capabilities through the
...
For OpenClaw, MCP is the primary interface. Claude Code, Cursor, and other MCP-compatible
agents connect to the OpenClaw MCP server and gain browser automation capabilities without
...
An MCP client (like Claude Code) launches OpenClaw as a subprocess and communicates over
...
Since OpenClaw uses standard MCP over stdio, any MCP-compatible client can connect to it.
The protocol flow:

1. Spawn the OpenClaw binary with `mcp` as the subcommand.
