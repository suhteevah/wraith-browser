...
A purpose-built browser engine strips away everything an agent does not need and optimizes for what it does. [Wraith Browser](/docs/getting-started/installation) takes this approach. It is a Servo-derived rendering engine compiled into a single 15 MB binary. No Chrome dependency, no browser drivers, no multi-hundred-megabyte downloads.
...
**143 MCP tools.** Every capability is exposed through the [Model Context Protocol](/docs/mcp-tools), so any MCP-compatible client, Claude Code, Cursor, or your own agent, can drive the browser natively. Check the [full tool reference](/docs/mcp-tools) for the complete catalog.
