...
## Project Status
🟢 **BR-6 + BR-7 + BR-8 + BR-9 all shipped** — react-select dropdowns commit, browse_fill propagates to React state, and reCAPTCHA v3 is unblocked via **cdp-attach** (new engine_type that attaches to the operator's daily Chrome on `--remote-debugging-port=9222` — real fingerprint passes v3 natively, no third-party service). 2captcha solver kept as autonomous fallback. Fresh binary at `target/release/wraith-browser.exe` (47 MB, mtime 9:33 AM). Open: BR-3 (PAT rotation, Matt-action), FR-4 (ClaudioOS bridge).

## What Was Done This Session
- **BR-9 primary fix — cdp-attach** (engine_cdp.rs + tools.rs + server.rs): new `CdpEngine::attach(port, target_filter)` constructor that connects to a running Chrome at `--remote-debugging-port` instead of spawning headless. Probes `/json/version` for fast-fail, lists page targets, picks by URL/title substring filter or first-available. Stores `chrome_process: None` so shutdown is a no-op (never kills the operator's browser). `Drop` and `shutdown()` updated to skip `Browser.close` in attach mode. Wired into `browse_session_create` as `engine_type: "cdp-attach"` (aliases `cdp_attach` / `attach`) with optional `attach_port` (default 9222) and `attach_target` (URL/title filter). Tool description updated. Matt's daily Chrome has real fingerprint + cookies + history → reCAPTCHA v3 scores it ≥0.7 naturally → submit fires. Zero third-party services.
- **BR-9 fallback — 2captcha solver retained** (server.rs + greenhouse-apply.yml): kept the proper v3 `grecaptcha.execute` monkey-patch injection and `solve_captcha` playbook action from earlier in the session. Useful for fully-autonomous runs without an operator Chrome — gracefully fails when `TWOCAPTCHA_API_KEY` is unset so the playbook continues. cdp-attach is the recommended path. Fresh binary 47 MB mtime 9:33 AM.
- **BR-8** (engine_cdp.rs, `BrowserAction::Fill`): rewrote Fill to use real CDP `Input.insertText` as primary path with the JS-setter as fallback. Fixes both reported bugs: 8a (prototype-lookup short-circuit always picked HTMLInputElement → textareas threw) and 8b (DOM-only fills didn't propagate to React state on controlled components → submit silently bailed). Added dual DOM/React-state verification so Fill returns `Failed` instead of false-positive Success.
...
- **Attach-mode shutdown is a no-op.** `CdpEngine::attach` builds an engine with `chrome_process: None`; `shutdown()` and `Drop` check that flag and skip `Browser.close` + process kill — your Chrome window stays open across MCP restarts. Don't add an unconditional kill in either path or you'll close the operator's browser unexpectedly.
- **`browse_fill` now verifies BOTH DOM value AND React state** before returning Success. Any agent/playbook code that ignored the Fill return value will now surface real failures it was previously eating. Audit any swarm/playbook code that bulk-fires `browse_fill` on Greenhouse-style controlled forms.
