...
## Project Status
🟢 **BR-6 + BR-7 + BR-8 + BR-9 phase 1 all shipped** — react-select dropdowns commit, browse_fill propagates to React state, and Greenhouse invisible reCAPTCHA v3 is now solved + injected via `grecaptcha.execute` monkey-patch (recommend option 1 from BR-9 triage). Greenhouse-apply playbook wired with the new `solve_captcha` step. Awaits Matt-action: `setx TWOCAPTCHA_API_KEY <key>` + Claude Code restart. Fresh binary at `target/release/wraith-browser.exe` (47 MB, mtime 8:43 AM). Open: BR-9 phase 2/3 (cdp-attach, Camoufox-Chrome) deferred, BR-3 (PAT rotation Matt-action), FR-4 (ClaudioOS bridge).

## What Was Done This Session
- **BR-9 phase 1** (server.rs + greenhouse-apply.yml): rewrote `browse_solve_captcha` reCAPTCHA v3 token injection to monkey-patch `grecaptcha.execute → Promise.resolve(TOKEN)` (the only path that works for invisible v3 — old code only filled a textarea that doesn't exist on v3). Extracted shared captcha logic into `solve_and_inject_captcha` helper method. Added `solve_captcha` action to the `swarm_run_playbook` executor. Inserted the step into both `playbooks/greenhouse-apply.yml` (file-on-disk version) and the embedded `PLAYBOOK_GREENHOUSE` constant. Awaits `TWOCAPTCHA_API_KEY` env via `setx`. Fresh binary 47 MB mtime 8:43 AM.
